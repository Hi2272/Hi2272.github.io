...
const unsigned long randomPixelsInterval = 100;  // ms Aktualisierungsintervall
...
void handleRoot() {
  ...
  html += "    <button onclick=\"sendRandomPixels()\">Zufallspixel</button>\n";
  html += "    <hr>\n";
  ...
  html += "    function sendRandomPixels(){ fetch('/randompixels').then(r=>r.text()).then(t=>updateStatus(t)).catch(e=>updateStatus('Fehler beim Senden')); }\n";
  html += "  </script>\n";
  html += "</body>\n";
  html += "</html>\n";
...
void setup() {
  ...
  server.on("/mic", HTTP_GET, handleMicToggle);              // Mic toggle endpoint
  server.on("/randompixels", HTTP_GET, handleRandomPixels);  // Zufallspixel
...
void loop() {
  ...
  // dynamische Helligkeit: wenn Mic-Kontrolle aktiv, wird auf 0  ..brightness gemappt
  uint8_t dynamicBrightness = brightness;
  if (micControlEnabled && !isScrolling) {
    dynamicBrightness = readMicLevel(brightness);
    matrix.setBrightness(dynamicBrightness);
  }
  ...
  // Zufallspixel-Modus: zeichnet regelmäßig zufällige Pixel gemäß Mic-Pegel
  if (randomPixelsActive) {
    unsigned long now = millis();
    if (now - lastRandomUpdate >= randomPixelsInterval) {
      lastRandomUpdate = now;

      // Misst Mikrofonpegel und mappt auf 0..255
      int level = readMicLevel(255);  // 0..255
      int totalPixels = MATRIX_WIDTH * MATRIX_HEIGHT;
      int count = constrain(level, 0, totalPixels);
      ...
      for (int i = 0; i < count; i++) {
        uint16_t color = matrix.Color(155 + random(100), 155 + random(100), 155 + random(100));
        matrix.setPixelColor(random(256), color);
      }
      matrix.show();
    }
  }
...
void handleRandomPixels() {
  // Unterbreche Scroll falls nötig und stoppe andere Modi
  stopScrollRequested = true;
  rainbowActive = false;
  colorModeActive = false;
  randomPixelsActive = true;
  // kurz warten, damit stop verarbeitet werden kann
  delay(10);

  server.send(200, "text/plain", "Zufallspixel eingeschaltet.");
}

// Handler für Mikrofonsteuerung (AN/AUS)
void handleMicToggle() {
  String on = server.arg("on");
  micControlEnabled = (on == "1");


  // Sofortige Anpassung der Matrix-Helligkeit: bei Aktivierung auf aktuellen Mic-Wert,
  // bei Deaktivierung zurück auf den eingestellten brightness-Wert.
  if (micControlEnabled) {
    uint8_t v = readMicLevel(brightness);  // 0..brightness
  ...
  String resp = String("Mikrofonsteuerung: ") + (micControlEnabled ? "AN" : "AUS");
  Serial.println(resp);
  server.send(200, "text/plain", resp);
}
